...
denoise_isomiR_counts = function(rowdata, count_df, transition_probability_matrix,
  ...
  while(niter <= max_iter & no_change < 1){
    ...
    if(setequal(unique_partitions, unique(update_df$partition))==TRUE){
      ...
      cat("Allowing all sequences to move to final partition \n")
      isomiR_seqs = filter(partition_df, center == 0) %>% select(.,uniqueSequence) %>% unlist() %>% unname()
      final_partition_df = partition_df
      
      #for each isomiR sequence:
      for(seq in isomiR_seqs){
        #pull out all the lambdas that we need 
        lambdas_for_seq = list()
        for(j in 1:length(master_lambdas)){
          if(!(seq %in% names(master_lambdas[[j]]))){
            center_seq = filter(partition_df, partition == j & center == 1) %>% select(., uniqueSequence) %>% unlist() %>% unname()
            a = Biostrings::pairwiseAlignment(pattern = center_seq, seq)
            t = get_transitions(a)
            l = compute_lambda(t, transition_probs)
            lambdas_for_seq[[j]] = l 
          }
          else{
            lambdas_for_seq[[j]] = master_lambdas[[j]][[seq]]
          }
        }
        lambdas_for_seq = unlist(lambdas_for_seq)
        new_p = which(lambdas_for_seq == max(lambdas_for_seq))
        if(length(new_p) > 1){
          new_p = sample(new_p, 1)
        }
        final_partition_df$partition[final_partition_df$uniqueSequence == seq] = new_p
      }
      return(list(partition_df=partition_df, final_partition_df = final_partition_df, initial_center_seq=initial_center_seq, alignments=alignments, 
  ...
  #at the end, we're going to let everything move to a final partition:
  cat("Allowing all sequences to move to final partition \n")
  isomiR_seqs = filter(partition_df, center == 0) %>% select(.,uniqueSequence) %>% unlist() %>% unname()
  final_partition_df = partition_df
