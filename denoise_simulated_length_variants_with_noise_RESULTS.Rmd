...
#mirna_w_noise = strsplit(sample_results_w_noise_files, "_simulated")[[1]][1]
num_fp_noise = sample_results_w_noise$num_false_positives
fp_rate_noise = num_fp_noise/(num_fp_noise + sample_results_w_noise$num_true_negatives)
false_positive_noise_df = data.frame(x=1:100, y=fp_rate_noise)
ggplot(false_positive_noise_df, aes(x=x,y=y)) + geom_point() + xlab("Dataset index") + ylab("Number of false positives") + ggtitle(paste0("False positive rate in each dataset for miRNA ", mirna, "w/ noise")) + scale_x_continuous(limits=c(0,100)) + scale_y_continuous(limits=c(-0.05, 0.05)) + geom_abline(slope=0, intercept=0.05, col="hotpink1")
```

```{r}
#load all the results objects: 
# results_all = lapply(sample_results_files, function(x) return(paste0("/scratch/hswan/thesis_isomiR_count_denoising/sims/01_09_2025/results/", x) %>% readRDS()))
# results_all_w_noise 
# 
...
# mirna_names = strsplit(sample_results_files, "_simulated")
...
### Getting results from correctly simulated no noise data containing only length variants that are shorter than the center sequencce 
```{r, echo = FALSE}
