...
partition_objs = readRDS(paste0(partition_obj_path, partition_obj_files[1]))
miRNA = strsplit(partition_obj_files[1], "_partition")[[1]][1]
...
partitioning = partition_objs[[idx]]$partition_df

center_seqs = data_list[[idx]]$center_seqs
isomiR_seqs = data_list[[idx]]$isomiR_seqs
...
true_isomiR_seqs = data_list[[idx]]$isomiR_seqs
...
filename = paste0("infer_counts_mat_dataset_", idx, ".Rds", collapse="")
