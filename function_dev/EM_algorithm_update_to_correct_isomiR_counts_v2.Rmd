...
estimate_isomiR_partition_membership = function(partition_df, transition_probs, max_iter_partitioning, OMEGA_A){
  ...
  while(iter_partitioning < max_iter_partitioning & no_change == 0){
    ...
    if(length(uniq_partitions) == 1){
      ...
      if(length(significant_seqs)>0){
        ...
        for(i in isomiR_seqs){
          l = lapply(master_lambdas, function(x) return(x[[i]])) %>% unlist() %>% unname()
          l = which(l == max(l))
          update_df$partition[update_df$uniqueSequence == i] = l
        }
  ...
  return(list(center_seqs=center_seqs, isomiR_seqs=isomiR_seqs, results_df=results_df, partition_df=partition_df, iter_partitioning=iter_partitioning))
}
```

```{r}
tst = estimate_isomiR_partition_membership(initial_partition_df, transition_probs=transition_probs, max_iter_partitioning = 2, OMEGA_A=0.05)
```
